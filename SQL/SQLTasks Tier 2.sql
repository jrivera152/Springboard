...
FROM Members AS m1
WHERE m1.joindate = (SELECT MAX(m2.joindate) FROM Members AS m2);
...
SELECT facid AS facility_ID, extract(month from starttime) AS month, SUM(slots) AS facility_usage
FROM Bookings
WHERE memid != 0
